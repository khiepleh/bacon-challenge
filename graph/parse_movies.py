...
def parse(csv_path: str, out_path: str):
    ...
    print('[parse_movies] Finished parsing {} rows into {} in {} seconds'.format(
        i, out_path, time.time() - start))
...
def build_connected_graph(actors_path: str) -> Graph:
    ...
    print('[parse_movies] Built connected graph in {} seconds'.format(
        time.time() - start))
...
def main():
    parser = argparse.ArgumentParser(
        description='Parse the Movies Dataset credits.csv into a form which is readily-consumable for the Bacon Challenge.')
    parser.add_argument(
        'csv', help='The credits.csv file containing the original Movies Dataset actor information.')
    parser.add_argument('-p', '--parsed-output', default='preprocessed/actors.jsonl',
                        help='Path to the desired output file for parsed data; by default, preprocessed/actors.jsonl.')
    ...
    if not os.path.exists(out):
        try:
            ...
            print(
                '[parse_movies] Could not create output directory; will try to open output file anyway')
