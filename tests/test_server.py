...
tmp_actors = os.path.join(os.path.dirname(os.path.abspath(server.g_actors_file)), 'tmp_actors.jsonl')
...
def test_bacon_simple(client):
    r = client.get(api_bacon, query_string={'actor' : 'Kevin Spacey', 'other' : 'params', 'do not' : 'matter'})
    j = r.get_json()
...
def test_bacon_missing_param(client):
    r = client.get(api_bacon, query_string={'but we' : 'need', 'the param': 'actor'})
    j = r.get_json()
...
def test_arbitrary_simple(client):
    r = client.get(api_arbitrary, query_string={'target': 'Kevin Costner', 'root' : 'Kevin Spacey', 'other' : 'params', 'do not' : 'matter'})
    j = r.get_json()
...
def test_arbitrary_same(client):
    r = client.get(api_arbitrary, query_string={'target': 'Kevin Costner', 'root' : 'Kevin Costner'})
    j = r.get_json()
...
def test_arbitrary_root_not_in_data(client):
    r = client.get(api_arbitrary, query_string={'target': 'Kevin Costner', 'root' : 'Anomander Rake'})
    j = r.get_json()
...
def test_arbitrary_target_not_in_data(client):
    r = client.get(api_arbitrary, query_string={'root': 'Kevin Costner', 'target' : 'Dassem Ultor'})
    j = r.get_json()
...
def test_arbitrary_neither_in_data(client):
    r = client.get(api_arbitrary, query_string={'root': 'Brys Beddict', 'target' : 'Trull Sengar'})
    j = r.get_json()
...
def test_arbitrary_missing_target(client):
    r = client.get(api_arbitrary, query_string={'root' : 'Kevin Spacey', 'other' : 'params', 'do not' : 'matter'})
    j = r.get_json()
...
def test_arbitrary_missing_root(client):
    r = client.get(api_arbitrary, query_string={'target' : 'Kevin Spacey', 'other' : 'params', 'do not' : 'matter'})
    j = r.get_json()
...
def test_multi_simple(client):
    r = client.get(api_multi, json=[['Kevin Bacon', 'Kevin Spacey'], ['Kevin Spacey', 'Kevin Costner'], ['Kevin Costner', 'Kevin Smith']])
    j = r.get_json()

    assert r.status_code    == 200
    assert j['Results']     == [['Kevin Bacon', 'Kevin Spacey', 2], ['Kevin Spacey', 'Kevin Costner', 2], ['Kevin Costner', 'Kevin Smith', 2]]
...
def test_multi_malformed_some_okay(client):
    in_data = [['foo', 'bar'], ['mux', 'qaz'], ['Kevin Bacon', 'Kevin Spacey', 'Kevin Costner'], ['big', 'foot']]
    r       = client.get(api_multi, json=in_data)
    j       = r.get_json()
...
def test_new_simple(client):
    new_movies = {
          'Gardens of the Moon' : ['Ganoes Paran', 'Whiskeyjack', 'Fiddler', 'Tattersail', 'Anomander Rake']
        , 'Deadhouse Gates'     : ['Fiddler', 'Cutter', 'Apsalar', 'Icarium', 'Mappo Runt']
    }
    ...
    r = client.get(api_arbitrary, query_string={'target': 'Ganoes Paran', 'root' : 'Icarium'})
    j = r.get_json()
